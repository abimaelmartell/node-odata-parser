...
it ('should parse uuid okay', function () {
  const uuid = '7b4b55a7-9ef8-426f-9c1f-1df388d488b6';
  var ast = parser.parse (`$filter=status eq ${uuid}`);
  assert.equal (ast.$filter.right.type, 'literal');
  console.log (ast.$filter.right);
  assert.equal (ast.$filter.right.value.flat (Infinity).join (''), uuid);
});
